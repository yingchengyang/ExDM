...
class MazeDreamerObsWrapper:
    def __init__(self, env, fix_init=False):
        self._env = env
        self._ignored_keys = []
        self._obs_spec = specs.Array((self._env.state_size,), np.float32, 'observation')
        self._action_spec = specs.BoundedArray((self._env.action_size,), np.float32,
                                               -self._env.action_range, self._env.action_range,
                                               'action')
        self.discount = 1
    ...
    def step(self, action):
        ...
        assert self.discount in (0, 1)
        obs = {
            'reward': self._env.reward.numpy().item(),
            'is_first': False,
            'is_last': self._env._state['done'],
            'is_terminal': self._env._state['done'],
            'observation': self._env._state['state'].numpy(),
            'action': action,
...
def make(name, obs_type, frame_stack, action_repeat, seed):
    ...
    # maze or maze_fix
    if domain == 'maze':
        if task.split('_', 1)[0] == 'fix':
            fix_init = True
            task = task.split('_', 1)[1]
        else:
            fix_init = False
        
        if task in ['square_a', 'square_b', 'square_c', 'square_d', 'square_corridor', 'square_corridor2', 'square_bottleneck', 'square_large', 'square_tree']:
            env_params = {
                "n": 50,
                "maze_type": task,
        ...
        env = MazeDreamerObsWrapper(env, fix_init=fix_init)
        return env
