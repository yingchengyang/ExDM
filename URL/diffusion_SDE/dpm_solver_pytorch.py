...
class DPM_Solver:
    ...
    def sample_with_grad(self, x, steps=20, t_start=None, t_end=None, order=3, skip_type='time_uniform',
               method='singlestep', denoise=False, solver_type='dpm_solver', atol=0.0078,
               rtol=0.05,
               ):
        """
        copy from the function sample
        we del torch.no_grad() to keep grad during the sampling, which is needed in DQL
        """
        t_0 = 1. / self.noise_schedule.total_N if t_end is None else t_end
        t_T = self.noise_schedule.T if t_start is None else t_start
        device = x.device
