...
class ReplayBuffer(IterableDataset):
    def __init__(self, storage, max_size, num_workers, nstep, discount,
                 fetch_every, save_snapshot, save_fake=False):
    ...
    def _sample(self):
        ...
        if self.save_fake:
            fake_action = episode['fake_action'][idx - 1]
            fake_action_ = episode['fake_action'][idx + self._nstep - 1]
        ...
        if self.save_fake:
            return (obs, action, fake_action, fake_action_, reward, discount, next_obs, done, *meta)
        else:
...
def make_replay_loader(storage, max_size, batch_size, num_workers,
                       save_snapshot, nstep, discount, save_fake=False):
